...
import { ExternalLink, Github, Sparkles, ShieldCheck, FileText, Scale, Landmark, MessageSquareText, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function FeaturedProject({ isDark }) {
  ...
  const keyFeatures = Array.isArray(featuredProject.keyFeatures) ? featuredProject.keyFeatures : [];
  const techStack = Array.isArray(featuredProject.techStack) ? featuredProject.techStack : [];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex flex-col items-center text-center mb-12 space-y-3">
          ...
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight">
            {featuredProject.name ?? 'Civil_AI'} Civic Copilot Platform
          </h2>
          <p className={`text-base max-w-2xl ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
            {featuredProject.subtitle ?? 'AI-Powered Civic & Statutory Legal Assistance Platform for Indian Citizens'}
          </p>
        </div>

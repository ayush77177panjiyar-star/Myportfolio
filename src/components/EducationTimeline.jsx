...
import { GraduationCap, MapPin, CheckCircle2, Compass } from 'lucide-react';
import { portfolioData } from '../data/portfolioData';

export default function EducationTimeline({ isDark }) {
  const education = portfolioData?.education ?? {};
  const safeJourney = Array.isArray(portfolioData?.journey) ? portfolioData.journey : [];
        ...
        <div className={`rounded-3xl p-8 border mb-16 transition-all duration-300 ${
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-1">
                <div className="inline-flex items-center gap-2 text-xs font-mono text-emerald-400 bg-emerald-500/10 px-2.5 py-0.5 rounded-full border border-emerald-500/20">
                  ...
                  <span>{education.status ?? 'Ongoing'}</span>
                </div>
                <h3 className="text-2xl font-bold tracking-tight">{education.institution ?? 'Geeta University, Panipat'}</h3>
                <p className="text-sm font-semibold text-indigo-400">{education.degree ?? 'Bachelor of Technology (B.Tech) - Computer Science'}</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 text-xs font-mono text-gray-400 bg-gray-800/40 p-3 rounded-2xl border border-gray-800">
              ...
              <span>{education.location ?? 'Panipat, Haryana, India'}</span>
            </div>
          </div>
        </div>
